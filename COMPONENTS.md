...
| Identity Broker | Decide which delegations are permitted | `TOKEN_EXCHANGE_ALLOWED` events |
| Auth Broker | Hold the only signing capability | `DELEGATION_TOKEN_SIGNED` events carry the backend and key id |
...
- Only the Auth Broker can sign one; every other component can merely ask.
